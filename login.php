...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($errors)){
        ...
        try{
            ...
            $pdo->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION); // エラーモードに「警告」に設定
            // 入力されたメールアドレスを元にユーザー情報を取り出す
            $stmt = $pdo->prepare(" SELECT * FROM user WHERE mail = ? ");
            $stmt->execute(array($input["mail"]));
            $user = $stmt->fetch(PDO::FETCH_ASSOC); // 文字列キーによる配列としてテーブル所得
        ...
        // ユーザー情報を取り出せた　かつ　パスワードが一致すれば、セッションに代入し、マイページへ遷移
